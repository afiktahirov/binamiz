...
function sidebarColor(a) {
    // Get all icon elements within the sidebar
    ...
    icons.forEach(function (icon) {
        ...
        // icon.classList.add('bg-gradient-' + color);
    });

    var activeIcon = document.querySelector(
        ".navbar-vertical .navbar-nav .nav-link.active .icon-shape",
    );
    console.log(activeIcon);
    if (activeIcon) {
        activeIcon.classList.add("bg-gradient-" + color);
        activeIcon.classList.remove("icon");
    }

    // Mark the clicked color option as active
    document
...
function sidebarType(a) {
    ...
    if ((color == "bg-transparent" || color == "bg-white") && bodyWhite) {
        ...
        if (navbarBrand) {
            ...
            if (navbarBrandImg && navbarBrandImg.includes("logo-ct.png")) {
        ...
        if (navbarBrand) {
            var navbarBrandImg = navbarBrand.src;
            if (navbarBrandImg && navbarBrandImg.includes("logo-ct-dark.png")) {
                var navbarBrandImgNew = navbarBrandImg.replace(
                    "logo-ct-dark",
                    "logo-ct",
                );
...
function navbarFixed(el) {
    ...
    if (!navbar) {
        console.warn("Navbar element not found, cannot apply fixed setting");
        return;
    }
...
function navbarBlurOnScroll(id) {
    ...
    if (!navbarElement) {
        console.warn(
            `Element with id ${id} not found, cannot apply navbar blur`,
        );
        return;
    }

    if (navbarElement.getAttribute("data-scroll") !== "true") {
        return;
    }
